...
public class NowPlayingMovie implements Serializable {
	...
	// Returns a NowPlayingMovie given the expected JSON
	// Reads `title`, `year`, `synopsis`, `posters.thumbnail`,
	// `ratings.critics_score` and the `abridged_cast`
	public static NowPlayingMovie fromJson(JSONObject jsonObject) {
		NowPlayingMovie b = new NowPlayingMovie();
		try {
	...
	public static ArrayList<NowPlayingMovie> fromJson(JSONArray jsonArray) {
		ArrayList<NowPlayingMovie> businesses = new ArrayList<NowPlayingMovie>(jsonArray.length());
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			NowPlayingMovie business = NowPlayingMovie.fromJson(businessJson);
			if (business != null) {
